...
// Devolve o offset (relativo a fp) de uma variavel
    ...
    while (atual != NULL) {
        switch (atual->op) {
            case OP_LABEL:
                ...
                if (atual->arg1[0] != 'L' || (atual->arg1[0] == 'L' && !isdigit((unsigned char)atual->arg1[1]))) {
                    ...
                    if (atual->arg2 != NULL) {
                        ...
                        strncpy(params_copy, atual->arg2, sizeof(params_copy) - 1);
                        params_copy[sizeof(params_copy) - 1] = '\0';

                        char* nome_param = strtok(params_copy, ",");
                        int idx = 0;
                        while (nome_param != NULL) {
                            int offset = -12 - (idx * 4);
                            printf("    STR r%d, [fp, #%d]\n", idx, offset);
                            strcpy(TABELA_MEMORIA[idx].nome, nome_param);
                            TABELA_MEMORIA[idx].offset = offset;
                            idx++;
                            nome_param = strtok(NULL, ",");
                        }
                        total_variaveis = idx;
                        offset_atual = -12 - ((idx - 1) * 4);
                    }
                }
                break;
            case OP_ASSIGN:
